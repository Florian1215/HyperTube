#!/usr/bin/env bash
#
# CLI demo walkthrough for the 42 authentication user story.
#
# User story:
#   As a visitor, I want to choose 42 sign-in, be redirected to the 42
#   authorization screen, and return safely to the app if authorization is
#   cancelled or malformed.
#
# Usage:
#   ./verification/user_stories/scripts/demo_42_auth_story.sh
#
...
#
# Examples:
#   BASE_URL=http://localhost:8080 ./verification/user_stories/scripts/demo_42_auth_story.sh
#   CURL_TIMEOUT=10 ./verification/user_stories/scripts/demo_42_auth_story.sh
...
print_summary() {
  ...
  heading 7 "Summary"
  explain "The CLI reports which 42 auth story checks succeeded, failed, or were skipped."
...
explain "The CLI checks whether the backend is reachable before starting the 42 auth walkthrough."
...
heading 2 "Start 42 Sign-In"
explain "The CLI calls the backend route that the frontend uses when a visitor clicks Sign In with 42."
...
if [[ "$HTTP_STATUS" == "302" ]]; then
  ...
  if [[ -n "$AUTHORIZATION_URL" && -n "$OAUTH_STATE" && "$STATE_COOKIE" != "$STATE_COOKIE_NAME=" ]]; then
    record_result "Start 42 sign-in" "OK" "Backend issued a 42 authorization redirect and state cookie."
    print_result "OK" "Backend issued a 42 authorization redirect and state cookie."
  else
    record_result "Start 42 sign-in" "FAIL" "Redirect response was missing Location, state, or state cookie."
  ...
  record_result "Start 42 sign-in" "WARN" "42 provider credentials are not configured; continuing with callback safety checks."
  print_result "WARN" "42 provider credentials are not configured; continuing with callback safety checks."
else
  record_result "Start 42 sign-in" "FAIL" "Unexpected response from 42 login start."
  print_result "FAIL" "Unexpected response from 42 login start."
  abort_critical "The 42 login start route did not behave as expected."
fi

heading 3 "Inspect Authorization Redirect"
explain "The CLI verifies that the redirect points at the official 42 authorization endpoint with an authorization-code state."

if [[ -n "$AUTHORIZATION_URL" ]]; then
  printf '\n%sAuthorization URL:%s\n' "$BOLD" "$RESET"
  printf '  %s\n' "$AUTHORIZATION_URL"

  if [[ "$AUTHORIZATION_URL" == https://api.intra.42.fr/oauth/authorize* && "$AUTHORIZATION_URL" == *"response_type=code"* && "$AUTHORIZATION_URL" == *"state=$OAUTH_STATE"* ]]; then
    record_result "Inspect authorization redirect" "OK" "Redirect targets 42 OAuth with response_type=code and state."
    print_result "OK" "Redirect targets 42 OAuth with response_type=code and state."
  else
    ...
    abort_critical "The 42 authorization redirect was malformed."
  fi
else
  record_result "Inspect authorization redirect" "SKIP" "Skipped because 42 credentials are not configured."
  print_result "SKIP" "Skipped because 42 credentials are not configured."
fi

heading 4 "Cancel 42 Consent"
explain "The CLI simulates 42 redirecting back with an access_denied error and the correct state cookie."
CANCEL_URL="$BASE_URL/api/v1/auth/42/callback?error=access_denied&state=$OAUTH_STATE"
...
if { is_redirect_status "$HTTP_STATUS" || [[ "$HTTP_STATUS" == "401" ]]; } && response_has_error_code "OAUTH_DENIED"; then
  record_result "Cancel 42 consent" "OK" "Backend returned an OAuth denied error for the frontend to display."
  print_result "OK" "Backend returned an OAuth denied error for the frontend to display."
else
  record_result "Cancel 42 consent" "FAIL" "Backend did not return the expected OAuth denied error."
  print_result "FAIL" "Backend did not return the expected OAuth denied error."
  abort_critical "The cancelled 42 callback was not handled safely."
fi
...
MISSING_STATE_URL="$BASE_URL/api/v1/auth/42/callback"
...
MISMATCH_URL="$BASE_URL/api/v1/auth/42/callback?code=fake-code&state=wrong-state"
