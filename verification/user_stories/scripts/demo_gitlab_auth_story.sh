#!/usr/bin/env bash
#
# CLI demo walkthrough for the GitLab authentication user story.
#
# User story:
#   As a visitor, I want to choose GitLab sign-in, be redirected to GitLab
#   authorization, and return safely to the app if authorization is cancelled
#   or malformed.
#
# Usage:
#   ./verification/user_stories/scripts/demo_gitlab_auth_story.sh
#
...
#
# Examples:
#   BASE_URL=http://localhost:8080 ./verification/user_stories/scripts/demo_gitlab_auth_story.sh
#   CURL_TIMEOUT=10 ./verification/user_stories/scripts/demo_gitlab_auth_story.sh
...
STATE_COOKIE_NAME="hypertube_oauth_gitlab_state"
...
print_summary() {
  ...
  heading 7 "Summary"
  explain "The CLI reports which GitLab auth story checks succeeded, failed, or were skipped."
...
explain "The CLI checks whether the backend is reachable before starting the GitLab auth walkthrough."
...
heading 2 "Start GitLab Sign-In"
explain "The CLI calls the backend route that the frontend uses when a visitor clicks Sign In with GitLab."
LOGIN_URL="$BASE_URL/api/v1/auth/gitlab/login"
...
if [[ "$HTTP_STATUS" == "302" ]]; then
  ...
  if [[ -n "$AUTHORIZATION_URL" && -n "$OAUTH_STATE" && "$STATE_COOKIE" != "$STATE_COOKIE_NAME=" ]]; then
    record_result "Start GitLab sign-in" "OK" "Backend issued a GitLab authorization redirect and state cookie."
    print_result "OK" "Backend issued a GitLab authorization redirect and state cookie."
  else
    record_result "Start GitLab sign-in" "FAIL" "Redirect response was missing Location, state, or state cookie."
  ...
  record_result "Start GitLab sign-in" "WARN" "GitLab provider credentials are not configured; continuing with callback safety checks."
  print_result "WARN" "GitLab provider credentials are not configured; continuing with callback safety checks."
else
  record_result "Start GitLab sign-in" "FAIL" "Unexpected response from GitLab login start."
  print_result "FAIL" "Unexpected response from GitLab login start."
  abort_critical "The GitLab login start route did not behave as expected."
fi

heading 3 "Inspect Authorization Redirect"
explain "The CLI verifies that the redirect points at the official GitLab authorization endpoint with the user profile scope."

if [[ -n "$AUTHORIZATION_URL" ]]; then
  printf '\n%sAuthorization URL:%s\n' "$BOLD" "$RESET"
  printf '  %s\n' "$AUTHORIZATION_URL"

  if [[ "$AUTHORIZATION_URL" == https://gitlab.com/oauth/authorize* && "$AUTHORIZATION_URL" == *"response_type=code"* && "$AUTHORIZATION_URL" == *"state=$OAUTH_STATE"* && "$AUTHORIZATION_URL" == *"read_user"* ]]; then
    record_result "Inspect authorization redirect" "OK" "Redirect targets GitLab OAuth with code flow, state, and read_user scope."
    print_result "OK" "Redirect targets GitLab OAuth with code flow, state, and read_user scope."
  else
    ...
    abort_critical "The GitLab authorization redirect was malformed."
  fi
else
  record_result "Inspect authorization redirect" "SKIP" "Skipped because GitLab credentials are not configured."
  print_result "SKIP" "Skipped because GitLab credentials are not configured."
fi

heading 4 "Cancel GitLab Consent"
explain "The CLI simulates GitLab redirecting back with an access_denied error and the correct state cookie."
CANCEL_URL="$BASE_URL/api/v1/auth/gitlab/callback?error=access_denied&state=$OAUTH_STATE"
...
if { is_redirect_status "$HTTP_STATUS" || [[ "$HTTP_STATUS" == "401" ]]; } && response_has_error_code "OAUTH_DENIED"; then
  record_result "Cancel GitLab consent" "OK" "Backend returned an OAuth denied error for the frontend to display."
  print_result "OK" "Backend returned an OAuth denied error for the frontend to display."
else
  record_result "Cancel GitLab consent" "FAIL" "Backend did not return the expected OAuth denied error."
  print_result "FAIL" "Backend did not return the expected OAuth denied error."
  abort_critical "The cancelled GitLab callback was not handled safely."
fi
...
MISSING_STATE_URL="$BASE_URL/api/v1/auth/gitlab/callback"
...
MISMATCH_URL="$BASE_URL/api/v1/auth/gitlab/callback?code=fake-code&state=wrong-state"
