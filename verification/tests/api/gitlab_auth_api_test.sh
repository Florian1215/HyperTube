...
# API smoke test for the GitLab OAuth route surface.
#
# This test is safe to run without real GitLab credentials:
# - If /auth/gitlab/login reports OAUTH_NOT_CONFIGURED, that path is accepted.
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hypertube-gitlab-auth-test.XXXXXX")"
...
test_login_start() {
  ...
  section "GitLab login start"

  request "GET" "/auth/gitlab/login"

  if [[ "$LAST_STATUS" == "503" ]]; then
    pass "GitLab login reports missing provider configuration"
    assert_error_envelope "GitLab login not configured response" "OAUTH_NOT_CONFIGURED" "OAuth provider GitLab is not configured"
    skip "GitLab authorization redirect checks" "GITLAB_CLIENT_ID, GITLAB_CLIENT_SECRET, or GITLAB_REDIRECT_URL is not configured"
    return 0
  fi

  if ! expect_status "GitLab login starts with redirect" "302"; then
    return 1
  fi
  ...
  if [[ "$location" == https://gitlab.com/oauth/authorize* ]]; then
    pass "GitLab login redirects to the official GitLab authorization endpoint"
  else
    fail "GitLab login redirects to the official GitLab authorization endpoint"
  fi

  [[ "$location" == *"response_type=code"* ]] && pass "GitLab authorize URL requests code flow" || fail "GitLab authorize URL requests code flow"
  [[ "$location" == *"read_user"* ]] && pass "GitLab authorize URL requests user profile scope" || fail "GitLab authorize URL requests user profile scope"
  [[ "$location" == *"client_id="* ]] && pass "GitLab authorize URL includes client_id" || fail "GitLab authorize URL includes client_id"
  [[ "$location" == *"redirect_uri="* ]] && pass "GitLab authorize URL includes redirect_uri" || fail "GitLab authorize URL includes redirect_uri"

  if [[ -n "$state" ]]; then
    pass "GitLab authorize URL includes state"
  else
    fail "GitLab authorize URL includes state"
  fi

  if [[ -n "$cookie_state" && "$cookie_state" == "$state" ]]; then
    pass "GitLab login stores matching state cookie"
    OAUTH_LOGIN_STATE="$state"
    OAUTH_LOGIN_COOKIE="$STATE_COOKIE_NAME=$cookie_state"
  else
    fail "GitLab login stores matching state cookie"
  fi

  assert_header_contains "GitLab state cookie is HttpOnly" "Set-Cookie" "HttpOnly"
  assert_header_contains "GitLab state cookie uses SameSite=Lax" "Set-Cookie" "SameSite=Lax"
}

test_callback_csrf_errors() {
  section "GitLab callback CSRF validation"

  request "GET" "/auth/gitlab/callback"
  assert_redirect_or_error "Callback rejects missing state" "400" "INVALID_OAUTH_STATE" "Invalid OAuth state"

  request "GET" "/auth/gitlab/callback?code=fake-code&state=wrong-state" "$STATE_COOKIE_NAME=expected-state"
...
test_callback_denial() {
  ...
  section "GitLab provider denial callback"

  state="${OAUTH_LOGIN_STATE:-simulated-state}"
  cookie="${OAUTH_LOGIN_COOKIE:-$STATE_COOKIE_NAME=$state}"

  request "GET" "/auth/gitlab/callback?error=access_denied&state=$state" "$cookie"
  assert_redirect_or_error "Callback handles denied GitLab consent" "401" "OAUTH_DENIED" "OAuth authorization was denied for GitLab"
