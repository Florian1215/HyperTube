...
export function patchUser(locale: string, data: string[], userId?: number | string) {
    ...
    return apiClient<iUserToken>(`users/${userId}`, locale, {method: "PATCH", body: JSON.stringify(updateData)});
}

export function postNewPassword(locale: string, data: string[]) {
    return apiClient<iUserToken>(`users/new-password`, locale, {method: "PATCH", body: JSON.stringify({current_password: data[0], new_password: data[1], new_password_confirm: data[2]})});
}
